    ...
    >
      ...
      <a-form-model-item>
        <a-select
          placeholder="请选择年级"
          style="width: 172px"
          v-model="table.select.grade"
        >
          <a-select-option v-for="list in getnjArr" :key="list" :value="list">
            {{ list }}
          </a-select-option>
        </a-select>
      </a-form-model-item>
      <a-form-model-item>
        <a-select
          placeholder="请选择班级"
          style="width: 172px"
          v-model="table.select.classname"
        >
          <a-select-option
            v-for="list in getCalssArr"
            :key="list"
            :value="list"
          >
            {{ list }}
...
export default {
  ...
  data() {
    return {
      getnjArr: ["一年级", "二年级", "三年级", "四年级", "五年级", "六年级"],
      getCalssArr: ["1班", "2班", "3班", "4班", "5班", "6班"],
      loadingVal: false,
      isPower: true,
      is_guest_status: 1,
      enrollId: "", //学生类别id
